...
<p style="font-family: Poppins, sans-serif; font-size: 14px; font-weight: normal; margin: 0; margin-bottom: 15px;">
    Hi {{ $document?->user?->first_name }},
</p>

@if ($document?->status == 'Sent')
<p style="font-family: Poppins, sans-serif; font-size: 14px; font-weight: normal; margin: 0; margin-bottom: 15px;">
    {{ $document->title }} has been sent to
    @if ($sendToMyself)
        yourself
    @else
    <b>{{ implode(', ', $documentParticpants) }}</b>
    @endif

</p>

<p style="font-family: Poppins, sans-serif; font-size: 14px; font-weight: normal; margin: 0; margin-bottom: 25px;">
    You will be notified as soon as the document is signed.
</p>
@endif
...
<p style="font-family: Poppins, sans-serif; font-size: 14px; font-weight: normal; margin: 0; margin-bottom: 15px;">
    <b>{{ $document->title }}</b>
    you shared has been signed by
    <b>{{ implode(', ', $document->participants->pluck('first_name')->toArray()) }}</b>. <br>
</p>
<p style="font-family: Poppins, sans-serif; font-size: 14px; font-weight: normal; margin: 0; margin-bottom: 15px;">
    The {{$document->status}} document is attached.
</p>
@endif
