...
<p style="font-family: Poppins, sans-serif; font-size: 14px; font-weight: normal; margin: 0; margin-bottom: 15px;">
  Hi {{ $participant?->document?->user?->first_name }},
</p>

@if (!empty($participant->comment))
<p style="font-family: Poppins, sans-serif; font-size: 14px; font-weight: normal; margin: 0; margin-bottom: 15px;">
    {{ $participant->first_name }}  {{ $participant->last_name }} has {{$participant->status}} the document, <b> {{ $participant->document->title }} </b> due to <i> {{ $participant->comment }} </i>
</p>
@else
<p style="font-family: Poppins, sans-serif; font-size: 14px; font-weight: normal; margin: 0; margin-bottom: 15px;">
    {{ $participant->first_name }}  {{ $participant->last_name }} has {{$participant->status}} the document, <b> {{ $participant->document->title }} </b>.
</p>
@endif
...
<p style="font-family: Montserrat, sans-serif; mso-line-height-rule: exactly; margin: 0; margin-bottom: 16px; margin-bottom: 24px;">
    Yours Credibly, <br> <b>The ToNote Team.</b>
</p>
@endcomponent
